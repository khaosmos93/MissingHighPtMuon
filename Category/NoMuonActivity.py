...
for name, fileName in files:
  print "\n\n\n",name," : ", fileName
  if not printKinematics: fn='NoMuonActivityAroundProbeEvents_'+name+'.txt'
  elif printKinematics: fn='NoMuonActivityAroundProbeKinematics_'+name+'.txt'
  fout = open(fn, 'wt')
  eventCount=0
  ...
  for event in events:
    ...
    for selTk in selectedTracks:
      numMuAroundTk = 0
      for muu in muons:
        if dR(muu,selTk) < 0.3: numMuAroundTk+=1
      if numMuAroundTk==0:
        numTkCategory+=1
