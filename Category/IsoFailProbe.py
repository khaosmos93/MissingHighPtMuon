...
for name, fileName in files:
  print "\n\n\n",name," : ", fileName
  if not printKinematics: fn='IsoFailProbeEvents_'+name+'.txt'
  elif printKinematics: fn='IsoFailProbeKinematics_'+name+'.txt'
  fout = open(fn, 'wt')
  eventCount=0
  ...
  for event in events:
    ...
    ### Category : At least one probe fails isolation ###
    numIsoFailTk=0
    categoryTracks=[]
    for selTk in selectedTracks:
      ...
      for tkiso in tracks:
        ...
        if abs(tkiso.dxy(PV.position())) > 0.1: continue
        if (dR(tkiso,selTk) < 0.3 and dR(tkiso,selTk) > 0.01):
          TkSumPt03 += tkiso.pt()
    ...
    if (vervos): print "number of tracks fail isolation in event: ",numIsoFailTk
    if numIsoFailTk < 1: continue
